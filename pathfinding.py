...
def construct_path(path, draw, current_node):
    while current_node in path:
        current_node = path[current_node]
        current_node.make_path()
        draw()

def dijkstras_algorithm(draw, grid, start, end, rows):
    found = False
    searching = True
    start.visited = True
    queue.append(start)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_d and start and end:
                    ...
                    dijkstras_algorithm(lambda: draw(win, grid, ROWS, width), grid, start, end, ROWS)
